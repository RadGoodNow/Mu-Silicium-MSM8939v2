...
#include "EfiIdoGpio.h"

EFI_IDO_GPIO_PROTOCOL *mGpioProtocol;
...
KeypadDeviceConstructor ()
{
  ...
  Status = gBS->LocateProtocol (&gEfiIdoGpioProtocolGuid, NULL, (VOID *)&mGpioProtocol);
  if (!EFI_ERROR (Status)) {
    // Configure keys
    /// Volume Up Button
    ...
    DEBUG ((EFI_D_ERROR, "%a: Failed to Locate Ido GPIO Protocol! Status = %r\n", __FUNCTION__, Status));
  }
  ...
  UINT64                  Delta)
{
  ...
  for (UINTN Index = 0; Index < (sizeof(KeyList) / sizeof(KeyList[0])); Index++) {
    ...
    IsPressed = !mGpioProtocol->GetPin ((IdoGpioBank *)Context->PinctrlBase, Context->BankOffset, Context->Pin);
